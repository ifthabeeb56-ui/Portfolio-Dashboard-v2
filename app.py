...
def update_prices(df):
    ...
    try:
        # സിംഗിൾ ടിക്കർ ആണെങ്കിലും ലിസ്റ്റ് ആണെങ്കിലും കൈകാര്യം ചെയ്യുന്നു
...
if not hold_df.empty:
    ...
    # ലാഭനഷ്ടങ്ങൾക്കനുസരിച്ച് നിറം മാറ്റാൻ (Updated to .map())
    def color_pnl(val):
...
with st.expander("➕ Add New Stock"):
    with st.form("add_stock_form"):
        c1, c2 = st.columns(2)
        with c1:
            name = st.text_input("Stock Symbol (eg: TCS.NS)").upper()
        ...
        if st.form_submit_button("Save to Portfolio"):
            if name:
                ...
                df.to_csv(PORTFOLIO_FILE, index=False)
                st.success(f"{name} added to {acc}")
                st.rerun()
